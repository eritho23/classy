...
func (app *ClassyApplication) RegisterRouteHandlers(router *http.ServeMux) {
	...
	router.HandleFunc("GET /scoreboard", app.GetScoreboardHandler)
	router.HandleFunc("GET /scoreboard/challenge/{challengeId}/complete", app.GetScoreboardChallengeCompleteHandler)
	router.HandleFunc("POST /scoreboard/challenge/{challengeId}/complete", app.PostScoreboardChallengeCompleteHandler)
